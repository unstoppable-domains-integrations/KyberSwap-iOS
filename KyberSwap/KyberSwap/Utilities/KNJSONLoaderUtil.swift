...
class KNJSONLoaderUtil {
  ...
  static func jsonDataFromFile(with name: String) -> JSONDictionary? {
    guard let path = Bundle.main.path(forResource: name, ofType: "json") else {
      if isDebug { print("---> Error: File not found with name \(name)") }
      return nil
    }
    ...
    do {
      ...
      if isDebug { print("---> Error: Get data from file path \(urlPath.absoluteString) failed with error \(error.localizedDescription)") }
      return nil
    }
    guard let jsonData = data else {
      if isDebug { print("---> Error: Can not cast data from file \(name) to json") }
      return nil
    }
    do {
      ...
      if isDebug { print("---> Error: Cast json from file path \(urlPath.absoluteString) failed with error \(error.localizedDescription)") }
      return nil
    }
    return nil
  }
